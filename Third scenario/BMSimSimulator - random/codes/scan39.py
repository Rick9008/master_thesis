...
import random
def Scan_function39(nodes, i, all_event, NODE_TIME, NODE_EVENT, reception_ratio, logger, Time):
    if Time < nodes[i].first_time_scan+nodes[i].SCAN_WINDOW:
        Num_AD_nodes = 0
        if TOTAL_LOG == 1:
            detail_log.info("scan39 node %s at Time %s ", i, all_event[i][NODE_TIME])
        for j in range(len(nodes[i].neighbors)):
            if nodes[nodes[i].neighbors[j]].advertisetag39 == 1:
                Num_AD_nodes += 1
                Advertise_node = nodes[i].neighbors[j]
        if nodes[i].enable_proposed_algo:
            nodes[i].check_ch_timer(Time)
        if Num_AD_nodes == 1:
            nodes[i].scanchannel39(nodes[Advertise_node].channel39)
            if random.randint(0, 100) > reception_ratio[nodes[i].ID][Advertise_node]:
                ...
                all_event[i][NODE_EVENT] = SCAN39_C_EVENT
                nodes[i].Scan_Time += SCAN_STEP
                return all_event[i][NODE_TIME], all_event[i][NODE_EVENT]
            # =================================================================
            # [新增] Year 1 Plan: 接收端邏輯 (Receiver Side)
            # 封包已成功接收，現在位於 cache 的最後一個位置
            received_packet = nodes[i].cache[len(nodes[i].cache) - 1]
            # 檢查我自己是否啟用了演算法
            if nodes[i].enable_proposed_algo:
                # 1. 讀取封包中的 src_role，更新鄰居計數
                # (須確保 message.py 中有 src_role 屬性)
                sender_role = getattr(received_packet, 'src_role', 'Legacy')
                nodes[i].update_neighbor_counts(sender_role)

                # 2. 執行被動分群 (判斷是否變身 CH/GW)
                nodes[i].run_passive_clustering(Time)

                # 3. Algorithm 1 中的 "Drop Non-DF Message if CH"
                # 如果我是 CH 且收到非 DF 封包，為了減少碰撞與節能，可以選擇丟棄
                if nodes[i].role == "CH" and getattr(received_packet, 'is_df_packet', False) == False:
            ...
            if (nodes[i].feature == JUST_GENERATION or (nodes[i].feature == FRIEND_NODE and L == False)) and \
                    f == False and nodes[i].cache[len(nodes[i].cache)-1].heartbeat == 0 and \
            ...
            if nodes[i].cache[len(nodes[i].cache)-1].friend_poll == 0 or \
                ...
                if len(nodes[i].buffer) < BUFFER_SIZE:
                        ...
                        if TOTAL_LOG == 1 and ((nodes[i].feature == JUST_RELAY) or
                            ...
                            logger[i].info('(relay)    %s    %s    %s    %s    %s    %s    %s',
                                           Advertise_node, nodes[i].buffer[len(nodes[i].buffer)-1].source,
                                           round(Time, 2),
                        ...
                        if f == True:  # the packet is received by destination
                              ...
                              if nodes[i].cache[len(nodes[i].cache)-1].friend_poll == 0 and TOTAL_LOG == 0 \
                                      and nodes[i].buffer[len(nodes[i].buffer)-1].heartbeat == 0:

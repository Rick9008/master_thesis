...
def update_configurations_models(nodes, NUMBER_NODES, NODE_RANGE, NUMBER_RELAY_NODE, NUMBER_RELAY_G_NODE, ENVIRONMENT,Gar):
    ...
    # === [新增] Year 1 Plan: 週期性執行演算法 2 (自適應 Duty Cycle) ===
    # 每次 Network Updater 執行時 (例如每 6秒)，我們更新一次 Duty Cycle
    for i_algo in range(NUMBER_NODES):
        if nodes[i_algo].enable_proposed_algo:
            # 傳入鄰居列表，計算並更新該節點的 Scan Window
            nodes[i_algo].calculate_adaptive_duty_cycle(nodes[i_algo].neighbors)
    # ================================================================
####### Our run time algorithms for determining relay and center nodes ########
    ...
    # with this part, the user can use algorithms  to determine the features of
        ...
        # [修改] 這裡原本是 30，改為 60 以符合 initializer.py 設定
            nodes[i_up].SCAN_INTERVAL = 60 #30
            # [修改] 保護機制：如果是支援演算法的節點，不要重設它的 Scan Window
            if not nodes[i_up].enable_proposed_algo:
                # 只有 Legacy 節點才強制重設為 60 (100% Duty Cycle)
                nodes[i_up].SCAN_WINDOW = 60
            # 如果是 Proposed Algo，保持它自己計算出的 Window 值，不覆蓋
            nodes[i_up].Advertise_Interval = 20
            nodes[i_up].SCAN_INTERVAL = 60 
            nodes[i_up].Advertise_Interval = 20

...
while True:  # events are executed in order of their time
    ...
    if all_event[i_node][NODE_EVENT] == HEARTBEAT_EVENT_Adv37:
        ...
        destination.remove(nodes[0].LOW_POWER_ID)
        destination.remove(nodes[46].LOW_POWER_ID)
    ...
    # in this event, the node generates a main message and advertises it on channel 37
        ...
        if nodes[i_node].n_count == 0:  # first transmission
            ...
            # === [新增] DF 路徑檢查邏輯 (Source Logic) ====================================
            packet_type = 0 # 預設為一般封包
            if nodes[i_node].enable_proposed_algo:
                # 假設 destination1 是一個 list，我們取第一個當目標
                target_node = destination1[0] 
                
                # 檢查 Forwarding Table 是否有此目的地的路徑
                if target_node in nodes[i_node].forwarding_table:
                    # 有路徑 -> 發送 DF_DATA
                    packet_type = 1 # DF_DATA
                    # print(f"Node {i_node} sends DF_DATA to {target_node}")
                else:
                    # 沒路徑 -> 發送 PATH_REQUEST 進行探索
                    packet_type = 2 # PATH_REQUEST
                    # 啟動 Discovery Timer (這裡簡化，直接發送請求)
                    # print(f"Node {i_node} starts Path Discovery for {target_node}")
                    
                    # 根據 Algorithm 1: 若我是 UC，隨機倒退後變身 (這裡簡化為直接設角色)
                    if nodes[i_node].role == "UC":
                         # 簡單實作：沒路徑就先嘗試成為 CH 發起請求
            ...
            if nodes[i_node].enable_proposed_algo:
                message_generation.src_role = nodes[i_node].role
            # ============================================================================
            #destination assignment for the generative nodes
            if i_node == 28:
                destination1=[1]
            if i_node == 7:
                destination1=[2]    
            message_generation = message(TTL, i_node, destination1, data)  # creating the main message
            # === [新增] Piggyback Source Role ===
            if nodes[i_node].enable_proposed_algo:
                message_generation.src_role = nodes[i_node].role
    ...
    # in this event, the relay node advertises its buffer's packets on channel 37 
        ...
        # === [新增] Piggyback Relay Role ===
        # 重要：轉發時，要將封包內的 src_role 改為我的角色
        # 這樣收到的人才知道是哪個角色的鄰居在傳送
        if nodes[i_node].enable_proposed_algo:
            nodes[i_node].message.src_role = nodes[i_node].role
        # ===================================

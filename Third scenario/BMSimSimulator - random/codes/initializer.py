...
#######  initial settings ##############
###### deployment setting ###########
NUMBER_NODES = 100  # the number of nodes in the network
x = []  # the positions of nodes in the network
y = []
#x = [39, 39, 11, 12, 1, 31, 30, 9, 22, 0, 21, 2, 3, 1, 6, 7, 8, 4, 23, 9, 14, 35, 22, 32, 19, 15, 14, 26, 27, 28, 29, 20, 34, 38, 13, 37, 35, 36, 37, 38, 39, 0, 1, 2, 3, 4, 5, 6, 7, 8]
#y = [6, 1, 43, 24, 3, 32, 37, 25, 7, 25, 12, 11, 1, 30, 9, 22, 0, 21, 2, 3, 1, 6, 7, 8, 4, 23, 9, 14, 33, 22, 32, 19, 15, 14, 26, 27, 28, 29, 30, 31, 32, 33, 4, 35, 6, 37, 38, 39, 0, 1, 22, 3, 4, 5, 6, 7]
...
#         #x.append(random.randint(1, ENVIRONMENT))
#         # x.append(9 * i17)
#         #y.append(random.randint(1, ENVIRONMENT))
#         # y.append(9 * j17)
##############################################

NUMBER_RELAY_NODE = 50  # number of nodes with relay feature in the network
NUMBER_RELAY_G_NODE = 50 # number of nodes with relay and generator features in the network
...
BUFFER_SIZE = sys.maxsize # the size of the nodes' buffer
...
# === [新增] Year 1 Plan: 60% Proposed Algo / 40% Legacy Setup ===
DF_RATIO = 0.6
...
# Algo Flag: 100% True (因為您說不支援 DF 的節點也會分群)
# 除非您要跑 Baseline (全 False)，否則這裡設為 True
algo_enabled = True
# ====================================================================
...
for i1 in range(NUMBER_NODES):  
   # 取得該節點的 DF 能力設定
    ...
    # 建立節點：傳入兩個開關
    # 1. enable_df: 控制是否能建立 DF 路徑
    # 2. enable_proposed_algo: 控制是否執行分群與 Duty Cycle
    nodes.append(node(i1, x[i1], y[i1], 
                      enable_df=is_df_capable, 
                      enable_proposed_algo=algo_enabled))  # the positions of the nodes are add to them
...
for i_r in range(NUMBER_NODES):
    ########## node setting ###########
    nodes[i_r].SCAN_INTERVAL = 60  # 30
    nodes[i_r].SCAN_WINDOW = 60  # 30
    nodes[i_r].Relay_Retransmit_Count = 2
    nodes[i_r].Network_Transmit_Count = 2
    nodes[i_r].Rris = 1
    nodes[i_r].Ntis = 1
    ...
    nodes[i_r].time_heartbeat = 6000 #random.randint(0, 1000)

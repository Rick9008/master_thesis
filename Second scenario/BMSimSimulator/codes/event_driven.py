...
def apply_df_next_hop(sender_node, pkt):
    """
    DF-unicast: 不改 pkt.destination（保留 final destination）
    只在 pkt 上放:
      - pkt.df_final: 最終目的地
      - pkt.df_next_hop: 這次要送到的 next hop
    """
    if not getattr(sender_node, "enable_df", False):
        return
    pkt_type = getattr(pkt, "type", 0)
    if pkt_type != 1:  # 只對 DF_DATA 做 unicast
        return
    # 只處理「單一最終目的地」的封包
    if not hasattr(pkt, "df_final"):
        if isinstance(pkt.destination, list) and len(pkt.destination) == 1:
            pkt.df_final = pkt.destination[0]
        else:
            return  # 多播/廣播先不做 DF-unicast
    final_dest = pkt.df_final
    # 查表：next hop
    nh = sender_node.forwarding_table.get(final_dest, None)
    if nh is None:
        # 沒路徑就退回 flooding（讓 scan 端不做 next-hop filter）
        if hasattr(pkt, "df_next_hop"):
            delattr(pkt, "df_next_hop")
        return
    pkt.df_next_hop = nh
#=====================================================================================
...
while True:  # events are executed in order of their time
    ...
    # in this event, the node generates a main message and advertises it on channel 37
         ...
         # === 新增：只允許 data source 產生資料 / DF path ===
        if not getattr(nodes[i_node], "is_data_source", False):
    ...
    ####################################################
        ...
        Time = all_event[i_node][NODE_TIME]
    ##################################################################
# ===================== DF path visualization (after simulation ends) =====================
picked = None
for origin_id, n in enumerate(nodes):
    lane_nodes_map = getattr(n, "df_lane_nodes", {})
    if lane_nodes_map:
        target_id = next(iter(lane_nodes_map.keys()))
        picked = (origin_id, target_id)
        break

if picked is None:
    print("[DF] No DF paths established; nothing to draw.")
else:
    o, t = picked
    print(f"[DF] Drawing ALL lanes for origin={o}, target={t} in ONE figure.")
plot_and_print_df_paths(nodes, Gar)
# ==========================================================================================
